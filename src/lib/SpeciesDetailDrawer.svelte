...
<script lang="ts">
  ...
  import { ArrowLeft, Settings, EllipsisVertical, Trash2, X, Camera, Loader } from 'lucide-svelte'
  ...
  export async function loadSpecies(speciesId: string) {
    try {
      species = await pb.collection('species').getOne<Species>(speciesId)
      fetchPlants(speciesId)
      formData = species
    } catch (error) {
      species = null
...
<Drawer title={species?.name || 'Loading...'}>
  {#snippet actions()}
